...
for freq in [0,0.1,0.25,1,2.5,5,10,15,100,1000]:
    ...
    plt.plot(values[2],[(3.3*(200*((y+1)/2)+50)/255) for y in values[1]],'--',label="Entrada")
    ...
    values[0]=errorDelete([((y*10e6))/950 for y in values[0]],10)
    ...
    xs,smooth=smoothSignal([values[2],values[0]],0.5,30)
    ...
    plt.title("Frquência de entrada = "+str(freq)+" Rad/s")
    plt.legend()
    plt.grid(True)
    plt.show()
